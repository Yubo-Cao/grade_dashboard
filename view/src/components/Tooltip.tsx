...
const Tooltip: React.FC<TooltipProps> = ({
    ...
    const getTooltipClasses = () => {
        let baseClasses = 'absolute text-slate-900 dark:text-slate-200 rounded-lg shadow-lg p-2 bg-white dark:bg-slate-800 z-10';
        const animationClasses = animated
            ? 'opacity-0 scale-90 transform transition-opacity'
            : '';
        const visibleClasses = isVisible ? 'opacity-100 scale-100' : 'opacity-0';
